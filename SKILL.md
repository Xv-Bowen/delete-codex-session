...
- Treat every path, schema, reference, integrity, ownership, open-session, current-session, and unsupported-storage diagnosis as a safety warning with an affected scope and disposition.
...
- Never delete rows from an unknown schema object. UUIDs found in unknown storage only protect candidates from deletion. If an extension contains an approved target reference or cannot be inspected within the bounded contract, preserve the affected database component and continue independent safe work.
...
- Let the currently selected capable model interpret and explain the complete structured compatibility assessment without keying policy to a model name or version. Model judgment may accept evidence-backed additive compatibility and recommend a scope, but it never supplies mutation SQL, overrides protected references, declares an opaque format safe without evidence, or converts an unknown reference into deletion authority.
...
- `active` or `pending_preserved`: do not interfere; report the current action.
...
When present, also read the selected `state_database`, `logs_database`, and `paginated_history_database_plan`, their schema-compatibility records, the paginated row counts and frozen target-row contract, `desktop_catalog_schema_compatibility`, `auxiliary_thread_database_plans`, historical `extension_protected_ids`, accepted newer versions, scan completeness, and target-reference hits. Explain material protection or ambiguity without dumping schema internals or row hashes.
...
- exact pre-existing job directories, if any, selected for strict success cleanup, supported empty-historical recovery, or bounded predecessor linking; every other audited entry remains preserved

Do not ask separately for scope selection, staging, the private job path, final deletion, Ghostty launch, or successful metadata cleanup. Bind the response internally to the selected report-time semantic approval-scope fingerprint and never expose it. The fingerprint is built from an explicit allowlist: selected roots and options, recursive target membership, executable and retained targets, exact approved object locators and identities, selected database paths with stable main-file identity, protected references that affect those targets, and the exact selected historical snapshot. Unselected historical totals, process IDs, preflight diagnostics, unknown-but-non-owning schema inventory, SQLite `-wal`/`-shm` state, file sizes/mtimes, and other runtime observations are execution evidence rather than user authority and must not invalidate an otherwise unchanged approval. Exact selected target rows, target membership, exclusions, and selected historical identities remain authority boundaries. A later macOS Automation/TCC prompt is separate system consent and may still appear once.
